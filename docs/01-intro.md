...
Данный раздел задаёт рамку всего проекта:
он объясняет зачем нужна трансформация, какие проблемы решает, какая целевая архитектура формируется и на каких принципах строится проект.